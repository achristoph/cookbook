...
3. any variable that you want to test inside function/method will need to be returned so that the its value can be tested, however, there are variables that are 'in the middle' that is produced by some logic that should be tested - this logic needs to be extracted out as a method so that it can be tested in isolation.
